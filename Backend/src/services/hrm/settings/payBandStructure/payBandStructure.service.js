...
import {
  ...
  replacePayBandStructureRepo,
} from "../../../../repositories/hrm/settings/payBandStructure/payBandStructure.repository.js";

const verifyPayBand = async ({ schoolSlug, payBandSlug }) => {
  const payBand = await findStructurePayBandRepo({ schoolSlug, payBandSlug });
...
export const getPayBandStructureService = async ({ schoolSlug, payBandSlug }) => {
  await verifyPayBand({ schoolSlug, payBandSlug });
  return getPayBandStructureRepo({ schoolSlug, payBandSlug });
};

export const savePayBandStructureService = async ({ schoolSlug, payBandSlug, payload }) => {
  await verifyPayBand({ schoolSlug, payBandSlug });
  ...
  for (const row of earningRows) {
    const baseAmount = row.calculationBase === "GROSS_EARNING" ? grossEarning : basicPay;
    const calculatedAmount = row.calculationType === "PERCENT" ? (Number(row.value) / 100) * baseAmount : Number(row.value);
    grossEarning += calculatedAmount;
  ...
  for (const row of deductionRows) {
    const baseAmount = row.calculationBase === "GROSS_EARNING" ? grossEarning : basicPay;
    let calculatedAmount = row.calculationType === "PERCENT" ? (Number(row.value) / 100) * baseAmount : Number(row.value);

    if (row.deductionType.maximumValue !== null && calculatedAmount > Number(row.deductionType.maximumValue)) {
      calculatedAmount = Number(row.deductionType.maximumValue);
    }
  ...
  return replacePayBandStructureRepo({ schoolSlug, payBandSlug, rows: finalRows });
};
